...
/*
 ...
 C言語でおなじみのmalloc/freeはヒープ領域にメモリを確保/開放する関数である。
 またC++にはnew/deleteという演算子があり、これもヒープ領域にメモリを確保/開放するために使われる。
 スタック領域と異なり、PCに搭載されたメモリ容量の限界までメモリを使用することができる。
 一方、ヒープ領域に確保されたメモリはdeleteを使って解放しないとメモリリークが発生するなど、管理が難しい。
 さらにヒープ領域は確保・開放が遅いため、安易に使うとプログラムの速度低下に繋がりうる。

 実はstd::stringやstd::vector、std::mapも内部的にはヒープ領域にメモリを確保している。
 そのため、これらは非常に柔軟で大きなデータの格納も可能であるが、その分メモリ確保・開放のオーバーヘッドがある。
 関数の引数としてstd::vectorなどを渡すときに参照渡しをするのは、このヒープ領域の再確保や開放のオーバーヘッドを避けるためである。
*/
