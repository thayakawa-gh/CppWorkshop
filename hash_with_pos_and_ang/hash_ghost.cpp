...
/*
 問題
 1. 現在のハッシュ化は誤差に合わせたビン幅10umの設定になっています。
    この値を変更して、速度がどのように変化するか調べてみてください。

 2. 上のコードはGhost filterを意図したハッシュ化となっています。
    これをt2lのようにLinkletを生成するコードに修正してみましょう。
    basetrack_pl48.txt、basetrack_pl49.txtを読み込み、以下の条件で接続されるLinkletを生成してください。

    1. xy座標、角度のaffine変換の必要はありません。z座標はともに0になっていますが、PL48側のz座標は1000と見做してください。
    2. 位置ずれはPL48とPL49の中央面（z=500）まで飛跡を外挿して比較し、xy方向それぞれ50umまで許容します。
    3. 角度ずれはxy方向それぞれ0.2radまで許容します。
    4. ハッシュ化はPL49に対して適用します。まずは座標による2次元のハッシュ化で十分です。

    Linkletは異なるPL間で飛跡を比較するので、Basetrack座標による素直なハッシュ化では難しくなります。
    ハッシュ化の基準とする座標は、PL48と49の中央面（z=500）まで飛跡を外挿したときの座標を用いるとよいでしょう。
    外挿したときの座標cx、cyは、NETSCANの座標系の場合、次のようになります。
    PL48: cx = x - 500 * ax, cy = y - 500 * ay
    PL49: cx = x + 500 * ax, cy = y + 500 * ay

    なお、通常t2lの際は相対corrmap（corrmap-align.lstなど）に記録された区画ごとに異なるaffine parameterを与えて座標変換を行うため、
    飛跡の読み込み、座標変換、ハッシュ化、接続処理は区画ごとに逐一行うべきですが、
    今回は簡単のため全飛跡が単一の区画に収まるものとします。

 
    あっという間にできてしまった人は、ax、ayを組み込んだ4次元ハッシュ化に挑戦してみましょう。
    ただし、4次元にしても速くなるとは限りません。フィルムの飛跡密度や接続条件に依存するので、むしろ遅くなってしまうこともありえます。
