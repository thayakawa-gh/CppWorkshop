...
// 1. E:\udd\hayakawa\prg\ThirdPartyにあるyaml-cpp-0.8.0フォルダを、任意のローカルなフォルダなどにコピペする。
      GitHubから直接入手しても良いが、CMakeを用いてビルドする必要があるため知識がないと大変。Eドライブのものはビルド済み。
// 2. Visual Studioでyaml-cppを使いたいプロジェクトを開く。
// 3. ソリューションエクスプローラー（大抵はVisual Studioのウィンドウ右側にあります）から、プロジェクト名を右クリックし、プロパティを選択。
// 4. 「構成プロパティ」→「VC++ ディレクトリ」→「インクルードディレクトリ」に、コピーしたyaml-cppフォルダ内のincludeフォルダへのパスを追加する（yamlcpp1.png）。
// 5. 「構成プロパティ」→「VC++ ディレクトリ」→「ライブラリディレクトリ」に、コピーしたyaml-cppフォルダ内のlibフォルダへのパスを追加する（yamlcpp1.png）。
// 6. 「構成プロパティ」→「リンカー」→「入力」→「追加の依存ファイル」にて、
//    左上をDebugにした状態でyaml-cppd.libを追加し右下の「適用」をクリック、その後左上をReleaseに変更してyaml-cpp.libを追加する（yamlcpp2.png）。
